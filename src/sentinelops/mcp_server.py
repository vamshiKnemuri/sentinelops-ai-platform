...
def search_runbooks(query: str, service: str = "unknown") -> list[dict]:
    ...
    with traced(
        "sentinelops.mcp.search_runbooks",
        **{"tool.name": "search_runbooks", "service.name": service},
    ):
...
def get_workload_health(service: str, namespace: str = "default") -> dict:
    ...
    with traced(
        "sentinelops.mcp.get_workload_health",
        **{"tool.name": "get_workload_health", "service.name": service},
    ):
        signal = IncidentSignal(
            source="mcp",
